...
function detectType(name: string): AnimType {
...
function drawFrame(ctx: CanvasRenderingContext2D, phase: number, type: AnimType) {
  ...
  // Base geometry (at rest)
  const headR = 16;
  let bx = CX;        // body x offset
  let by = 0;         // body y offset
  let rotation = 0;

  // Limb tips (relative to canvas, not offset yet)
  ...
  // Derived positions (applying body offset)
  const headCY = 60 + by;
  const headCX = bx;
...
export function MoveDemoModal({ move, bpm = 120, onClose }: MoveDemoModalProps) {
  const t = useTranslations("move");
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const rafRef = useRef<number>(0);
  const startRef = useRef<number | null>(null);
  const animType = detectType(move.name);
